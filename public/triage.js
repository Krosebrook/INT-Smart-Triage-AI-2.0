...
export function processTriageRequest(ticketData) {
    const { issueDescription, customerTone, ticketSubject, customerName } = ticketData;
    ...
    const highPriorityKeywords = ['down', 'outage', 'critical', 'urgent', 'broken', 'not working', 'crashed', 'hack', 'breach', 'security', 'data loss'];
    const mediumPriorityKeywords = ['slow', 'issue', 'problem', 'error', 'bug', 'migration', 'update'];
    const lowPriorityKeywords = ['question', 'help', 'how to', 'feature', 'enhancement', 'inquiry'];

    // INT service keywords for department routing
    const infoSecKeywords = ['security', 'compliance', 'soc2', 'gdpr', 'cyber', 'breach', 'vulnerability', 'insurance', 'audit'];
    const techKeywords = ['server', 'network', 'email', 'cloud', 'backup', 'it', 'computer', 'software', 'saas', 'migration'];
    const webKeywords = ['website', 'web design', 'ecommerce', 'shopify', 'wordpress', 'landing page', 'hosting'];
    const brandKeywords = ['logo', 'brand', 'identity', 'design', 'visual', 'color', 'typography'];
    const contentKeywords = ['content', 'writing', 'seo', 'blog', 'copy', 'ebook', 'article'];
    const marketingKeywords = ['marketing', 'hubspot', 'crm', 'automation', 'campaign', 'email marketing'];
    const operationsKeywords = ['bookkeeping', 'accounting', 'process', 'workflow', 'ai', 'bi', 'analytics'];
    ...
    const responseApproach = generateResponseApproach(customerTone, priority, department);
...
function generateResponseApproach(tone, priority, department) {
    ...
    const priorityAddition = priority === 'high' ? ' Escalate to senior team member if needed. Follow up within 1 hour.' : '';
...
function generateTalkingPoints(tone, priority, category) {
    ...
    const tonePoints = {
        angry: ['I sincerely apologize for any inconvenience this has caused.', 'Your frustration is completely understandable.'],
        urgent: ['We recognize the time-sensitive nature of this request.', 'I\'m escalating this for immediate attention.'],
        frustrated: ['I understand how frustrating this must be.', 'We\'re committed to making this right.'],
        confused: ['Let me clarify that for you in simple terms.', 'Feel free to ask any follow-up questions.'],
        calm: ['I appreciate your patience as we work through this.', 'We\'re happy to provide any additional information you need.']
    };
