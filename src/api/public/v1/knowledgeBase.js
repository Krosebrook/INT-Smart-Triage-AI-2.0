...
export async function handleGenerateKnowledgeBaseArticle(req, res) {
  if (req.method !== 'POST') {
  ...
  try {
    const messagesSummary =
      messages
        ?.filter(message => message.sender_type === 'csr')
...
export default handleGenerateKnowledgeBaseArticle;
