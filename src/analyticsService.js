        ...
        return {
            ...
            data: {
                ...
                percentages: {
                    ...
                    medium: total > 0 ? ((distribution.medium / total) * 100).toFixed(1) : 0,
    ...
    try {
        ...
        return {
            ...
            data: Object.entries(workload).map(([department, stats]) => ({
                department,
                ...stats,
                utilization: stats.total > 0 ? ((stats.pending / stats.total) * 100).toFixed(1) : 0
            }))
        };
    } catch (error) {
    ...
    try {
        ...
        data.forEach(report => {
            ...
            if (report.status === 'resolved') {
                ...
                if (report.resolved_at) {
                    const resolutionTime = new Date(report.resolved_at) - new Date(report.created_at);
        ...
        return {
            ...
            data: Object.entries(metrics).map(([agent, stats]) => ({
                ...
                resolutionRate: stats.totalTickets > 0 ?
                    ((stats.resolved / stats.totalTickets) * 100).toFixed(1) : 0,
                avgResolutionTime: stats.resolvedCount > 0 ?
                    (stats.totalResolutionTime / stats.resolvedCount / (1000 * 60 * 60)).toFixed(1) : 0
            }))
        };
    } catch (error) {
    ...
    try {
        ...
        data.forEach(report => {
            if (report.assigned_at) {
                const responseTime = (new Date(report.assigned_at) - new Date(report.created_at)) / (1000 * 60);
                analysis[report.priority].times.push(responseTime);
                analysis[report.priority].count++;
            }
        });

        const calculateAvg = (times) => times.length > 0 ?
